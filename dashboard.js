(() => {
  ...
  function gridCellMarkup(cell, isSelected, mode, beltCode, columnNumber) {
    ...
    /* A populated cell holds two actions, so it can no longer be one button: the
       body takes or releases the activity, and the corner opens what it teaches.
       The wrapper keeps the belt colour and selection ring the cell already had. */
    if (mode === 'teacher' || mode === 'partner') return cell ? `<div class="motherGridCell ${color} hasCell hasPreview ${isSelected ? 'isSelected' : ''}"><button type="button" class="cellSelect" data-class-grid-cell="${cell.id}" aria-pressed="${String(isSelected)}" aria-label="${escapeHtml(`${label}; ${isSelected ? 'selected' : 'not selected'}`)}">${title}<em>${isSelected ? 'Selected' : 'Select'}</em></button><button type="button" class="cellInfo" data-cell-preview="${cell.id}" aria-label="${escapeHtml(`What ${cell.title} teaches`)}" title="What this teaches">i</button></div>` : `<div class="motherGridCell ${color} isEmpty">${title}</div>`;
  ...
  function renderMotherGrid({ mode, cells = state.motherGrid, selectedIds = new Set(), heading = 'Mother Grid', description = '' }) {
    ...
    return `<br><article class="toolCard motherGridCard"><div class="cardHeading"><div><p class="eyebrow">${escapeHtml(heading)}</p><h3>${mode === 'teacher' ? 'Select your class sub-grid' : mode === 'partner' ? 'Select this partner curriculum' : mode === 'admin' ? 'Edit the overall Grid' : 'My class Grid'}</h3><p class="helperText">${escapeHtml(description || defaultText)}</p></div><label class="gridZoomControl">Zoom<input type="range" min="0.8" max="1.35" step="0.05" value="${state.gridZoom}" data-grid-zoom-control></label></div><div class="motherGridViewport"><div class="motherGridCanvas" data-mother-grid-canvas style="--grid-scale:${state.gridZoom}; --grid-columns:${columns}"><div class="motherGridHeader"><span class="motherGridCorner">Belt</span>${headers}</div>${rows}</div></div>${previewDialog()}</article>`;
  }

  /* Empty until something is previewed. Filling it on demand rather than rendering
     every cell's detail up front keeps a 175-cell Grid to one dialog, and means
     opening one does not re-render the workspace and lose the scroll position. */
  function previewDialog() {
    return '<dialog class="cellPreview" id="cellPreview" aria-labelledby="cellPreviewTitle"></dialog>';
  }

  function projectLinks(project) {
    const links = [
      project.instructions_url ? `<a class="previewLink" href="${escapeHtml(project.instructions_url)}" target="_blank" rel="noopener noreferrer">Instructions</a>` : '',
      project.video_url ? `<a class="previewLink" href="${escapeHtml(project.video_url)}" target="_blank" rel="noopener noreferrer">Video</a>` : ''
    ].filter(Boolean).join('');
    return links || '<span class="previewLink isNone">In-person activity</span>';
  }

  function openCellPreview(cellId) {
    const dialog = document.getElementById('cellPreview');
    const cell = state.motherGrid.find((entry) => entry.id === cellId);
    if (!dialog || !cell) return;

    const beltName = (gridRows.find(([code]) => code === cell.belt_code) || [, cell.belt_code])[1];
    const projects = Array.isArray(cell.projects) ? cell.projects : [];
    const body = projects.length
      ? `<ol class="previewProjects">${projects.map((project) => `<li><strong>${escapeHtml(project.name || 'Untitled activity')}</strong><div class="previewLinks">${projectLinks(project)}</div></li>`).join('')}</ol>`
      : `<p class="emptyCopy">${escapeHtml(cell.instructions || 'This intersection has no activities in the current Mother Grid.')}</p>`;

    dialog.innerHTML = `<form method="dialog" class="previewShell">
      <header class="previewHeader belt${escapeHtml(cell.belt_code)}">
        <div><p class="eyebrow">${escapeHtml(beltName)} belt &middot; column ${escapeHtml(String(cell.column_number))}</p>
        <h2 id="cellPreviewTitle">${escapeHtml(cell.category || cell.title || 'Grid activity')}</h2></div>
        <button class="previewClose" value="close" aria-label="Close">&times;</button>
      </header>
      <div class="previewBody">
        <p class="previewLede">What a camper works through at the ${escapeHtml(beltName)} belt in ${escapeHtml(cell.category || 'this category')}${projects.length ? `, across ${projects.length} activit${projects.length === 1 ? 'y' : 'ies'}` : ''}.</p>
        ${cell.instructions && projects.length ? `<p class="previewInstructions">${escapeHtml(cell.instructions)}</p>` : ''}
        ${body}
      </div>
    </form>`;
    dialog.showModal();
  ...
  function bindCellPreview() {
    document.querySelectorAll('[data-cell-preview]').forEach((button) => button.addEventListener('click', (event) => {
      event.stopPropagation();
      openCellPreview(button.dataset.cellPreview);
    }));
  }
  ...
  async function loadAdminDashboard() {
    ...
    try {
      ...
      state.awaitingPassword = data || [];
    } catch (awaitingError) {
      console.warn('Pending password list unavailable', awaitingError?.message || awaitingError);
  ...
  /* Staff who exist but have not chosen a password yet. This is the answer to "the
     email never arrived": an account listed here exists, so the send is the problem;
     an account missing from here was never created, and no email was ever attempted. */
  function awaitingPasswordCard() {
    const rows = state.awaitingPassword;
    if (!rows.length) {
      return `<article class="toolCard"><div class="cardHeading"><div><p class="eyebrow">Staff access</p><h3>Everyone has set a password</h3>
        <p class="helperText">No teacher or administrator is waiting on a set-password link.</p></div></div></article>`;
    }
  ...
  function bindAdminEvents() { document.getElementById('adminRosterImportForm')?.addEventListener('submit', (event) => run(importStudentRoster, event)); document.getElementById('teacherCsvImportForm')?.addEventListener('submit', (event) => run(importTeacherCsv, event)); document.getElementById('motherGridImportForm')?.addEventListener('submit', (event) => run(importMotherGrid, event)); document.getElementById('partnerForm')?.addEventListener('submit', (event) => run(createPartner, event));
    ...
    bindCellPreview(); document.querySelectorAll('[data-class-grid-cell]').forEach((button) => button.addEventListener('click', () => run(() => applyPartnerDraftChange((draft) => { const id = button.dataset.classGridCell; if (draft.has(id)) draft.delete(id); else draft.add(id); }), null)));
